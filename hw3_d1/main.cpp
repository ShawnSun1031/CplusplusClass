...
#include <cstdlib>
using namespace std;
int inpatient(int&,int&,int&,int&);
int outpatient(int&,int&,int&,int&);
...
int main()
{
    ...
    if(choice==1)
    {
        total = inpatient(number_of_day, rate, charge_service, charge_medication);
        file1(number_of_day, rate, charge_service, charge_medication, total);
    }
    else if(choice==2)
    {
        total = outpatient(number_of_day, rate, charge_service, charge_medication);
...
int inpatient(int& number_of_day,int& rate,int& charge_service,int& charge_medication)
{
...
int outpatient(int& number_of_day,int& rate,int& charge_service,int& charge_medication)
{
    number_of_day = 0;
    rate = 0;
    cout << "\nWhat is the charges for hospital services? ";
